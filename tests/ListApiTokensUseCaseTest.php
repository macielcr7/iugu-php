...
use Iugu\Application\ApiTokens\ListApiTokensUseCase;
use Iugu\Infrastructure\Http\IuguHttpClient;
use Iugu\Domain\ApiTokens\ApiToken;
use Psr\Http\Message\ResponseInterface;

class ListApiTokensUseCaseTest extends TestCase
{
    public function testExecuteSuccess(): void
    {
        ...
        $httpClient->expects($this->once())
            ->method('get')
            ->with("/v1/{$accountId}/api_tokens")
            ->willReturn($responseMock);

        $useCase = new ListApiTokensUseCase($httpClient);
        $tokens = $useCase->execute($accountId);
